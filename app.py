...
room_runtime = defaultdict(lambda: {'hands': [], 'speaker_id': None, 'selected_id': None, 'screen_share_id': None})
sid_to_participant = {}

def utcnow():
    return datetime.now(timezone.utc)
def room_channel(code): return f'room:{code}'
def participant_channel(p): return f'participant:{p.join_token}'
def codeify(value): return (''.join(ch for ch in value.upper() if ch.isalnum())[:16] or secrets.token_hex(3).upper())
...
def ensure_schema():
    ...
    if 'meeting_room' in inspector.get_table_names():
        ...
        for name, ddl in [
            ...
            if name not in cols: stmts.append(ddl)
        for stmt in stmts: db.session.execute(text(stmt))
...
def tally_vote(vote, room):
    ...
    result = 'Em andamento'
    if not vote.active:
        approved = counts.get('Sim', 0) > counts.get('Não', 0) if vote.rule == 'simple_majority' else counts.get('Sim', 0) >= (eligible_count // 2 + 1) if vote.rule == 'absolute_majority' else counts.get('Sim', 0) >= ((online_count * 2 + 2) // 3)
        result = 'Aprovada' if approved else 'Não aprovada'
    details = []
    if not vote.secret:
        for row in rows:
            p = Participant.query.get(row.participant_id)
            if p: details.append({'name': p.full_name, 'option': row.option})
    return {'presentes': online_count,'aptos': eligible_count,'votaram': voted,'faltam': max(eligible_count - voted, 0),'counts': counts,'percentages': percentages,'result': result,'details': details}
...
def room_state(room):
    ...
    hand_ids = [pid for pid in runtime['hands'] if any(p.id == pid for p in participants)]
    runtime['hands'] = hand_ids
    rows = []
    for p in participants:
        rows.append({'id': p.id,'full_name': p.full_name,'display_name': p.display_name,'is_admin': p.is_admin,'online': p.online,'is_eligible': p.is_eligible,'can_speak': p.can_speak or p.is_admin or room.speech_mode == 'free','mic_blocked': p.mic_blocked or (not room.allow_microphone and not p.is_admin),'cam_blocked': p.cam_blocked or (not room.allow_camera and not p.is_admin),'hand_raised': p.id in hand_ids,'selected': runtime['selected_id'] == p.id,'speaking': runtime['speaker_id'] == p.id})
    ...
    if active_vote:
        ...
        vote_data.update({'id': active_vote.id,'title': active_vote.title,'options': active_vote.options,'secret': active_vote.secret,'rule': active_vote.rule,'active': active_vote.active})
    return {'room': {'id': room.id,'title': room.title,'code': room.code,'invite_url': url_for('join_token_page', token=room.invite_token, _external=True),'camera_url': url_for('camera_companion', code=room.code, _external=True),'camera_qr_url': url_for('camera_qr', code=room.code),'status': room.status,'allow_microphone': room.allow_microphone,'allow_camera': room.allow_camera,'speech_mode': room.speech_mode,'selected_id': runtime['selected_id'],'speaker_id': runtime['speaker_id'],'screen_share_id': runtime.get('screen_share_id'),'summary_text': room.summary_text or '','decisions_text': room.decisions_text or ''},'participants': rows,'vote': vote_data,'hands': hand_ids}

def current_admin(): return session.get('admin_ok') is True
def emit_room_state(code):
    room = MeetingRoom.query.filter_by(code=code).first()
    if room: socketio.emit('room_state', room_state(room), to=room_channel(code))
def get_or_create_named_participant(room, full_name, display_name, can_speak=False):
    ...
    participant = Participant(room_id=room.id, full_name=full_name, display_name=display_name, join_token=secrets.token_urlsafe(24), can_speak=can_speak, is_eligible=False)
    db.session.add(participant); db.session.commit(); return participant
...
def admin_login():
    if request.method == 'POST':
        if request.form.get('login','').strip() == ADMIN_LOGIN and request.form.get('password','').strip() == ADMIN_PASSWORD:
...
def create_room():
    ...
    room = MeetingRoom(title=title, code=code, invite_token=secrets.token_urlsafe(24), status='open', allow_microphone=True, allow_camera=True, speech_mode=speech_mode)
    db.session.add(room); db.session.flush()
    admin_p = Participant(room_id=room.id, full_name='Administrador CoopexCam', display_name='Administrador', join_token=secrets.token_urlsafe(24), is_admin=True, is_eligible=False, can_speak=True, online=False)
    db.session.add(admin_p); db.session.commit()
...
def toggle_status(code):
    if not current_admin(): return jsonify(ok=False), 403
    room = MeetingRoom.query.filter_by(code=code).first_or_404()
    ...
    db.session.commit(); emit_room_state(code)
...
def delete_room(code):
    ...
    db.session.delete(room); db.session.commit()
...
def participant_action(code, pid):
    ...
    runtime = room_runtime[room.code]
    if action == 'toggle_eligible': p.is_eligible = not p.is_eligible
    elif action == 'allow_speak':
        p.can_speak = True
        if p.id in runtime['hands']: runtime['hands'].remove(p.id)
        runtime['selected_id'] = p.id
    elif action == 'block_mic': p.mic_blocked = not p.mic_blocked
    elif action == 'block_cam': p.cam_blocked = not p.cam_blocked
    elif action == 'remove':
        old_channel = participant_channel(p)
        p.online = False; p.left_at = utcnow(); p.removed = True
        db.session.commit()
        socketio.emit('removed', {'reason': 'Você foi removido da sala.'}, to=old_channel)
        emit_room_state(room.code)
        return jsonify(ok=True)
    elif action == 'spotlight': runtime['selected_id'] = p.id if runtime['selected_id'] != p.id else None
    db.session.commit(); emit_room_state(room.code)
    return jsonify(ok=True)
...
def bulk_action(code):
    ...
    if action == 'eligible_all':
        for p in participants: p.is_eligible = True
    elif action == 'eligible_none':
        for p in participants: p.is_eligible = False
    elif action == 'mute_all':
        for p in participants: p.mic_blocked = True
    elif action == 'camera_off_all':
        for p in participants: p.cam_blocked = True
    elif action == 'mic_global': room.allow_microphone = not room.allow_microphone
    elif action == 'cam_global': room.allow_camera = not room.allow_camera
    elif action == 'speech_mode':
        room.speech_mode = 'free' if room.speech_mode == 'controlled' else 'controlled'
        if room.speech_mode == 'free':
            for p in participants: p.can_speak = True
    db.session.commit(); emit_room_state(room.code)
    return jsonify(ok=True)
...
def create_vote(code):
    ...
    if active: return jsonify(ok=False, message='Já existe votação em andamento.'), 400
    payload = request.json or {}
    options = [str(x).strip() for x in (payload.get('options') or ['Sim','Não','Abstenção']) if str(x).strip()]
    if not payload.get('title'): return jsonify(ok=False, message='Informe o título da votação.'), 400
    vote = VoteSession(room_id=room.id, title=payload.get('title', 'Votação'), options_csv='|'.join(options), rule=payload.get('rule', 'simple_majority'), secret=bool(payload.get('secret')), active=True)
    db.session.add(vote); db.session.commit(); emit_room_state(room.code)
    return jsonify(ok=True)
...
def end_vote(code):
    ...
    if not vote: return jsonify(ok=False, message='Sem votação ativa.'), 400
    vote.active = False; vote.ended_at = utcnow(); db.session.commit(); emit_room_state(room.code)
    return jsonify(ok=True)
...
def cast_vote(join_token):
    ...
    if p.removed: return jsonify(ok=False, message='Participante removido.'), 403
    room = MeetingRoom.query.get_or_404(p.room_id)
    vote = VoteSession.query.filter_by(room_id=room.id, active=True).first()
    if not vote: return jsonify(ok=False, message='Sem votação ativa.'), 400
    if not p.is_eligible: return jsonify(ok=False, message='Você não está apto a votar.'), 400
    option = (request.json or {}).get('option')
    if option not in vote.options: return jsonify(ok=False, message='Opção inválida.'), 400
    already = VoteRecord.query.filter_by(vote_session_id=vote.id, participant_id=p.id).first()
    if already: return jsonify(ok=False, message='Voto já registrado.'), 400
...
def export_xlsx(code):
    ...
    room = MeetingRoom.query.filter_by(code=code).first_or_404()
    wb = Workbook()
    ws = wb.active; ws.title = 'Presença'
    ws.append(['Nome completo', 'Primeira entrada', 'Última saída'])
    sub = db.session.query(AttendanceLog.participant_id, func.min(AttendanceLog.entered_at).label('first_in'), func.max(AttendanceLog.exited_at).label('last_out')).filter_by(room_id=room.id).group_by(AttendanceLog.participant_id).all()
    for item in sub:
        p = Participant.query.get(item.participant_id)
        if p: ws.append([p.full_name, str(item.first_in or ''), str(item.last_out or '')])
    ws_notes = wb.create_sheet('Resumo')
    ws_notes.append(['Resumo da reunião']); ws_notes.append([room.summary_text or '']); ws_notes.append([]); ws_notes.append(['Decisões']); ws_notes.append([room.decisions_text or ''])
    ...
    if vote:
        ...
        ws2.append(['Título', vote.title]); ws2.append(['Resultado', tally['result']]); ws2.append(['Presentes', tally['presentes']]); ws2.append(['Aptos', tally['aptos']]); ws2.append(['Votaram', tally['votaram']]); ws2.append(['Faltam', tally['faltam']]); ws2.append([]); ws2.append(['Opção', 'Total', 'Percentual'])
        for opt, count in tally['counts'].items(): ws2.append([opt, count, f"{tally['percentages'].get(opt, 0)}%"])
        if tally['details']:
            ws2.append([]); ws2.append(['Nome', 'Voto'])
            for detail in tally['details']: ws2.append([detail['name'], detail['option']])
...
def export_pdf(code):
    ...
    c.setFont('Helvetica-Bold', 14); c.drawString(40, y, f'CoopexCam - {room.title}'); y -= 24
    c.setFont('Helvetica', 10); c.drawString(40, y, f'Sala: {room.code}'); y -= 20
    c.setFont('Helvetica-Bold', 11); c.drawString(40, y, 'Presença consolidada'); y -= 18
    sub = db.session.query(AttendanceLog.participant_id, func.min(AttendanceLog.entered_at).label('first_in'), func.max(AttendanceLog.exited_at).label('last_out')).filter_by(room_id=room.id).group_by(AttendanceLog.participant_id).all()
    c.setFont('Helvetica', 9)
    for item in sub:
        p = Participant.query.get(item.participant_id)
        if not p: continue
        c.drawString(40, y, f'{p.full_name} | entrada: {item.first_in or "-"} | saída: {item.last_out or "-"}'[:112]); y -= 14
        if y < 90: c.showPage(); y = 800; c.setFont('Helvetica', 9)
    c.showPage(); y = 800
    c.setFont('Helvetica-Bold', 12); c.drawString(40, y, 'Resumo da reunião'); y -= 20
    c.setFont('Helvetica', 10)
    for line in (room.summary_text or '').splitlines() or ['']:
        c.drawString(40, y, line[:100]); y -= 14
    y -= 10; c.setFont('Helvetica-Bold', 12); c.drawString(40, y, 'Decisões'); y -= 20; c.setFont('Helvetica', 10)
    for line in (room.decisions_text or '').splitlines() or ['']:
        c.drawString(40, y, line[:100]); y -= 14
    vote = VoteSession.query.filter_by(room_id=room.id).order_by(VoteSession.id.desc()).first()
    if vote:
        tally = tally_vote(vote, room)
        c.showPage(); y = 800; c.setFont('Helvetica-Bold', 12); c.drawString(40, y, f'Votação: {vote.title}'); y -= 20
        c.setFont('Helvetica', 10); c.drawString(40, y, f'Resultado: {tally["result"]}'); y -= 16
        c.drawString(40, y, f'Presentes: {tally["presentes"]} | Aptos: {tally["aptos"]} | Votaram: {tally["votaram"]} | Faltam: {tally["faltam"]}'); y -= 20
        for opt, count in tally['counts'].items():
            c.drawString(40, y, f'{opt}: {count} ({tally["percentages"].get(opt, 0)}%)'); y -= 14
        if tally['details']:
            y -= 8
            for detail in tally['details']:
                c.drawString(40, y, f'{detail["name"]}: {detail["option"]}'[:110]); y -= 14
                if y < 70: c.showPage(); y = 800; c.setFont('Helvetica', 10)
    c.save(); buf.seek(0)
...
def on_join(data):
    ...
    if not p or p.removed:
        emit('removed', {'reason': 'Acesso inválido para esta sala.'}); disconnect(); return
    room = MeetingRoom.query.get_or_404(p.room_id)
    sid_to_participant[request.sid] = p.id
    join_room(room_channel(room.code)); join_room(participant_channel(p))
    p.online = True; p.left_at = None; db.session.commit()
    db.session.add(AttendanceLog(room_id=room.id, participant_id=p.id)); db.session.commit()
    emit('joined_ok', {'participant_id': p.id, 'room_code': room.code}); emit_room_state(room.code)
...
def on_signal(data):
    ...
    if not sender or sender.removed or not target or target.removed: return
    emit('signal', {'from_id': sender.id,'type': data.get('type'),'description': data.get('description'),'candidate': data.get('candidate'),'sender_name': sender.display_name}, to=participant_channel(target))
